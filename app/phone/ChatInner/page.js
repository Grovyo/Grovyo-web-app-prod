...
function Innerdata() {
  ...
  const handleSend = async () => {
    ...
    const scrollDownContent = () => {
      if (contentRef.current) {
        contentRef.current.scrollTop += 1000000; // Adjust the scroll speed as needed
      }
    };
    ...
    if (newContent) {
      setScrolling(true);
      const scrollInterval = setInterval(scrollDownContent, 100000); // Adjust the interval as needed
  ...
  return (
    <div className="w-[100%] h-[80vh] flex flex-col ">
      {/* header */}
      <div className="h-[60px] w-[100%] absolute bg-white flex flex-row shadow-sm">
        {/* profile */}
      ...
      <div
        ...
        className="w-[100%] h-[100%] bg-bgg bg-contain px-5 py-6 flex flex-col overflow-auto scrollbar-hide"
      >
        {data.map((d, i) => {
          return (
      ...
      {/* Keyboard */}
      <div className="h-[80px] absolute bottom-0 rounded-t-2xl bg-white w-[100%] white flex flex-row">
