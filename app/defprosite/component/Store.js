...
function Store(props) {
  ...
  const [id, setId] = useState(""); // Add loading state

  useEffect(() => {
    if (props.product) {
      setTimeout(() => {
        setProductt(props.product); // Corrected to use setProduct
        setIsLoading(false);
      }, 2000);
    }
    if (props.id) {
      setTimeout(() => {
        setId(props.id); // Corrected to use setId
        setIsLoading(false);
      }, 2000);
    }
