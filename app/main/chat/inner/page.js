...
function page() {
  ...
  return (
    <div className="w-[100%] pn:max-md:hidden h-screen bg-white flex  flex-col overflow-hidden">
      ...
      <div className="absolute right-0">
        <div
          className={`bg-blue-700 border-r-2 border-[#f9f9f9] dark:border-[#171717] md:min-w-[390px] md:[360px] duration-1000 h-screen  ${
            click ? "absolute z-0 right-[9vh]" : "absolute z-0 -right-[100vh]"
          }`}
        ></div>
      </div>
      {/* <div
        className={`duration-150 w-36 absolute h-[100vh] overflow-auto scrollbar-hide bg-red-200  
        ${click ? "ml-96" : "ml-[100vw] "}`}
      ></div> */}
    </div>
  );
}
